...
export const createElement = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      const newElement = await Element.create({ data: req.body });
...
export const updateImageOfElement = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      // const {imageUrl,width,height,static} = req.body;
      if (req.user.type !== "admin") {
      ...
      const updatedElement = await Element.update({
        where: {
          id: Number(req.params.elementId),
        },
        data: {
          imageUrl,
        },
